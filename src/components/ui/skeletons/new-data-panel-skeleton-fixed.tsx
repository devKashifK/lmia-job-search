...
interface NewDataPanelSkeletonFixedProps {
  className?: string;
}

export function NewDataPanelSkeletonFixed({ className = '' }: NewDataPanelSkeletonFixedProps) {
  return (
    <div className={`flex flex-col  h-[1200px] ${className}`}>
      <div className="flex flex-1 min-h-0">
        {/* Middle Section - NOC Job Description - Using exact calculation: w-[calc(100%-24rem)] */}
        <div className="flex-1 min-w-0 max-w-4xl">
          <NocJobDescriptionSkeleton className="h-full" />
        </div>

        {/* Right Sidebar - All Jobs - Fixed width: w-96 = 24rem = 384px */}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-center items-center gap-2">
            ...
            <div className="flex gap-1">
              <Skeleton className="h-8 w-8 rounded" />
              <Skeleton className="h-8 w-8 rounded bg-brand-100" /> {/* Current page */}
              <Skeleton className="h-8 w-8 rounded" />
