...
export function TrialTimer() {
  const { isTrialActive, remainingTime } = useControl();
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0, y: -20 }}
        className={`fixed top-4 right-4 z-50 px-4 py-2 rounded-lg shadow-lg border-2 backdrop-blur-sm transition-colors duration-300 ${
          isUrgent
