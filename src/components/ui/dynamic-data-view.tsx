...
import { Building2, Briefcase, Utensils, LayoutGrid, Table2, Settings2 } from "lucide-react";
...
import { LMIA, hotLeadsColumns, lmiaColumns } from "@/components/filters/column-def";
...
export default function DynamicDataView({
  ...
  const type: DataType = isValidType(dataConfig?.type) ? dataConfig.type : 'hotLeads';
  const columns = type === 'lmia' ? lmiaColumns : hotLeadsColumns;
  const pageSize = dataConfig?.pageSize ? parseInt(dataConfig.pageSize as string) : 60;
  const currentPage = dataConfig?.page ? parseInt(dataConfig.page as string) : 1;
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-[200px]">
                    {columns.map((column) => {
                      ...
                      const headerContent = typeof column.header === 'string' ? column.header : key;
                      return key ? (
                        <DropdownMenuCheckboxItem
                          key={key}
                        ...
                        (item: LMIA, idx: number) => {
                          ...
                          return (
                            <JobCard
                              ...
                              priorityOccupation={type === 'lmia' ? item.priority_occupation : undefined}
                              approvedPositions={type === 'lmia' ? item.approved_positions : undefined}
