...
export function EmployerInsightDialog({ isOpen, onClose, employerName, nocCodes, onPitch }: EmployerInsightDialogProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-lg p-0 overflow-hidden border-none bg-white text-gray-900 shadow-2xl rounded-[1.5rem]">
                ...
                <div className="relative p-5 space-y-4">
                    ...
                    <div className="flex items-start justify-between">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <div className="p-1.5 bg-brand-600 rounded-lg shadow-sm">
                                    <Building2 className="w-4 h-4 text-white" />
                                </div>
                                <Badge variant="outline" className="text-[9px] font-black border-brand-200 text-brand-600 bg-brand-50 px-2 py-0.5 uppercase tracking-widest">
                                    Strategic Intelligence
                                </Badge>
                                <div className="ml-auto bg-slate-900 px-1.5 py-0.5 rounded-md flex items-center gap-1 shadow-sm border border-slate-800 scale-90">
                                    <span className="text-[9px] font-black text-brand-500">{insights?.total_insights || 0}</span>
                                    <span className="text-[6px] font-black text-slate-500 uppercase tracking-tighter">Hits</span>
                                </div>
                            </div>
                            <h2 className="text-xl font-black tracking-tight uppercase text-gray-900 leading-tight">{employerName}</h2>
                    ...
                    {isLoading ? (
                        <div className="py-10 flex flex-col items-center justify-center gap-3">
                            ...
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Synthesizing...</p>
                        </div>
                    ) : (
                        <motion.div 
                            ...
                            <div className="bg-brand-600/[0.03] border-brand-500/10 p-4 rounded-xl border relative overflow-hidden group">
                                <div className="relative space-y-1.5">
                                    <div className="flex items-center gap-1.5 mb-0.5">
                                        ...
                                        <h4 className="text-[9px] font-black text-brand-600 uppercase tracking-widest">Agentic Insights Brief</h4>
                                    </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <div className="bg-gray-50/80 border border-gray-100 p-3 rounded-xl space-y-0.5">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">LMIA</span>
                                    </div>
                                    <p className="text-sm font-black text-gray-900">{insights?.lmia_count || 0}</p>
                                </div>
                                ...
                                <div className="bg-gray-50/80 border border-gray-100 p-3 rounded-xl space-y-0.5">
                                    ...
                                    <p className="text-sm font-black text-gray-900">{insights?.trending_count || 0}</p>
                                </div>

                                <div className="bg-gray-50/80 border border-gray-100 p-3 rounded-xl space-y-0.5">
                                    <div className="flex items-center gap-1.5">
                                        <Calculator className="w-3 h-3 text-blue-500" />
                                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Wage</span>
                                    </div>
                                    <p className="text-sm font-black text-gray-900">${insights?.avg_wage?.toFixed(1) || '28.5'}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <h3 className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Momentum</h3>
                                    </div>
                                        ...
                                        <div className="flex items-end justify-between h-full gap-1.5">
                                            {years.slice(0, 5).reverse().map(year => (
                                                <div key={year} className="flex-1 flex flex-col items-center gap-1 group">
                                                    ...
                                                    <span className="text-[7px] font-black text-gray-400 text-center">{year.toString().slice(-2)}</span>
                                                </div>
                                            ))}
                                            {years.length === 0 && (
                                ...
                                <div className="space-y-3">
                                    <div>
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            ...
                                            <h3 className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Titles</h3>
                                        </div>
                                        <div className="flex flex-wrap gap-1">
                                            {insights?.top_titles.slice(0, 3).map((title, idx) => (
                                                <span key={idx} className="bg-white border border-gray-100 text-gray-800 px-2 py-0.5 text-[8px] font-black rounded-lg uppercase shadow-sm">
                                                    {title}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            ...
                                            <h3 className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Hubs</h3>
                                        </div>
                                        <div className="space-y-1">
                                            {insights?.locations.slice(0, 2).map((loc, idx) => (
                                                <div key={idx} className="flex items-center gap-1.5 text-[9px] font-black text-gray-500 uppercase tracking-tight">
                    ...
                    <div className="pt-2 flex gap-2">
                        ...
                        <Button 
                            ...
                            className="flex-1 bg-brand-600 hover:bg-brand-700 text-white font-black uppercase tracking-widest text-[9px] h-9 rounded-xl shadow-lg shadow-brand-500/20 group"
                        >
