        ...
        <div id="scorecard-printable" className="p-10 bg-white print:p-0">
          ...
          <div className="flex justify-between items-start border-b-2 border-brand-600 pb-8 mb-8">
            <div className="space-y-4">
              <div className="space-y-1">
                <h1 className="text-4xl font-black text-gray-900 tracking-tight">{client.full_name}</h1>
            ...
            <div className="text-right space-y-2">
               <div className="flex items-center justify-end gap-2 mb-2">
                   {agencyProfile?.logo_url ? (
                       ...
                       <div className="px-3 py-1.5 bg-brand-600 text-white font-black rounded-lg text-sm">
               ...
               <p className="text-[9px] text-gray-400 italic">Protected by Recruitment Agreement</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-4 space-y-6">
               <div className="bg-brand-50/50 border border-brand-100 rounded-2xl p-6 text-center space-y-4">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-600">Candidate Score</h3>
                  ...
                  <div className="space-y-1">
                    <p className="text-xs font-black uppercase tracking-widest text-gray-900">{label}</p>
                    <p className="text-[9px] text-gray-400 font-medium">Placement Readiness Index</p>
                  </div>
               </div>

               <div className="space-y-4">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-2">Core Competencies</h3>
                  <div className="flex flex-wrap gap-2">
                    {(data.skills || "").split(',').slice(0, 15).map((skill: string, i: number) => (
                        <div key={i} className="px-2.5 py-1 bg-gray-50 border border-gray-200 rounded text-[10px] font-bold text-gray-700 capitalize">
                            {skill.trim()}
                        </div>
                    ))}
                  </div>
               ...
               <div className="space-y-4 pt-4">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-2">Quick Facts</h3>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-gray-50 rounded-lg"><Award className="w-3.5 h-3.5 text-brand-600" /></div>
                        <div>
                            <p className="text-[9px] text-gray-400 font-bold uppercase">Experience</p>
                            <p className="text-[11px] font-black">{data.experience || '0'} Total Years</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-gray-50 rounded-lg"><Building2 className="w-3.5 h-3.5 text-brand-600" /></div>
                        <div>
                            <p className="text-[9px] text-gray-400 font-bold uppercase">Last Company</p>
                            <p className="text-[11px] font-black truncate max-w-[120px]">{data.company || 'N/A'}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-gray-50 rounded-lg"><Star className="w-3.5 h-3.5 text-brand-600" /></div>
                        <div>
                            <p className="text-[9px] text-gray-400 font-bold uppercase">Education</p>
                            <p className="text-[11px] font-black truncate max-w-[120px]">Degree Verified</p>
                        </div>
                    </div>
                  </div>
               </div>
            ...
            <div className="col-span-8 space-y-8">
               ...
               <section className="space-y-4">
                  <h3 className="text-xs font-black uppercase tracking-widest text-gray-900 border-b border-gray-100 pb-2">Professional Experience</h3>
                    ...
                    {(data.work_experience || []).slice(0, 3).map((exp: string, i: number) => {
                        ...
                        return (
                            <div key={i} className="relative pl-8">
                                <div className="absolute left-0 top-1.5 w-3.5 h-3.5 bg-white border-4 border-brand-600 rounded-full z-10 shadow-sm" />
                                <div className="space-y-1">
                                    <div className="flex items-center justify-between gap-2">
                                        <h4 className="text-sm font-black text-gray-900 uppercase tracking-tight">{role}</h4>
                                        <Badge variant="outline" className="text-[9px] font-bold h-5 border-gray-200">{date.replace(')', '')}</Badge>
                                    </div>
               ...
               <section className="space-y-4">
                  <h3 className="text-xs font-black uppercase tracking-widest text-gray-900 border-b border-gray-100 pb-2">Educational Background</h3>
                        ...
                        return (
                            <div key={i} className="p-4 border border-gray-100 rounded-xl flex gap-3 bg-gray-50/20">
                                ...
                                <div className="min-w-0">
                                    <h4 className="text-xs font-black text-gray-900 leading-tight uppercase truncate">{degree}</h4>
               ...
               <div className="pt-8 text-center bg-gray-900 border-t-4 border-brand-600 p-8 rounded-b-3xl text-white">
                  <div className="flex items-center justify-center gap-2 mb-4">
                      <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                      <span className="text-sm font-black uppercase tracking-[0.2em]">Agency Recommended</span>
                  </div>
