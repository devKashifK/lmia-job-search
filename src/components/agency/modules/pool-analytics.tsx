...
export function PoolAnalytics({ currentScore }: PoolAnalyticsProps) {
    ...
    return (
        <Card className="p-4 border-gray-100 shadow-sm relative overflow-hidden bg-white">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-brand-50 rounded-lg text-brand-600">
                        <BarChart3 className="w-4 h-4" />
                    </div>
                    <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest leading-none pt-0.5">Competitive Pool Index</h3>
                </div>
                <Popover>
                    ...
                    <PopoverContent side="top" align="end" className="w-64 p-4 rounded-xl border-gray-100 shadow-xl bg-white/95 backdrop-blur-sm">
                        <h4 className="text-xs font-black text-gray-900 mb-2 border-b border-gray-50 pb-1 uppercase tracking-tighter">2024 ITA Benchmarks</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                {/* Score Gauge */}
                <div className="relative flex flex-col items-center justify-center p-2 rounded-2xl bg-gray-50 border border-gray-100/50">
                    <Gauge className="w-8 h-8 text-gray-200 absolute top-2 right-2 opacity-50" />
                    <div className="text-4xl font-black text-gray-900 tracking-tighter">{currentScore}</div>
                    <div className={cn("text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full mt-1 border", zone.bg, zone.color, zone.border)}>
                        {zone.label}
                    </div>
                </div>
                ...
                <div className="md:col-span-3 space-y-3">
                    <div className="flex items-center justify-between px-1">
                        <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">Current ITA Thresholds (2024)</span>
                        <Badge variant="outline" className="text-[8px] h-4 font-bold text-brand-600 bg-brand-50 border-brand-100">IRCC DATA SYNC</Badge>
                    </div>
                                ...
                                <div key={bench.category} className="space-y-1">
                                    <div className="flex items-center justify-between text-[10px] font-bold">
                                        <div className="flex items-center gap-1.5 min-w-0">
                                            <div className={cn("w-1.5 h-1.5 rounded-full", isPassing ? "bg-green-500" : "bg-gray-200")} />
                                            <span className="text-gray-600 truncate">{bench.label}</span>
                                        </div>
                                        <div className="flex items-center gap-3 shrink-0">
                                            <span className="text-gray-400">{bench.score}</span>
                                            <span className={cn(
                                                "min-w-[45px] text-right font-black tracking-tighter",
                                    ...
                                    <div className="h-1 w-full bg-gray-50 rounded-full overflow-hidden">
                                        <motion.div 
                                            ...
                                            className={cn("h-full rounded-full", isPassing ? "bg-green-500" : "bg-brand-500/40")}
                                        />
                                    </div>
                                </div>
                            );
