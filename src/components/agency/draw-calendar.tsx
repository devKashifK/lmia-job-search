        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-base font-bold text-gray-900 uppercase tracking-tight">IRCC Draw Calendar</h2>
                ...
                <a
                    ...
                    className="flex items-center gap-1.5 text-[10px] font-bold text-brand-600 hover:text-brand-700 uppercase tracking-widest transition-all"
                >
            ...
            {draws.length > 0 && (
                <div className="grid grid-cols-3 gap-4">
                    <div className="bg-brand-900 text-white p-5 rounded-xl relative overflow-hidden">
                        <div className="absolute -top-4 -right-4 w-20 h-20 bg-white/5 rounded-xl" />
                        <p className="text-[9px] font-bold text-brand-300 uppercase tracking-widest mb-1">Latest Draw CRS</p>
                        <p className="text-3xl font-bold">{draws[0]?.min_crs}</p>
                        <p className="text-[10px] text-brand-400 mt-1 font-bold">{draws[0]?.draw_type}</p>
                    </div>
                    <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-xl">
                        <p className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest mb-1">Eligible Now</p>
                        <p className="text-3xl font-bold text-emerald-700">{draws[0]?.eligible_count || 0}</p>
                        <p className="text-[10px] text-emerald-600 mt-1 font-bold">clients ready for latest draw</p>
                    </div>
                    <div className="bg-amber-50 border border-amber-100 p-5 rounded-xl">
                        <p className="text-[9px] font-bold text-amber-600 uppercase tracking-widest mb-1">Within 50 Pts</p>
                        <p className="text-3xl font-bold text-amber-700">{draws[0]?.near_eligible_count || 0}</p>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-16 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <p className="text-sm font-bold text-slate-400">No Draw Data Available</p>
                </div>
            ) : (
                    ...
                    {draws.map((draw, idx) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={cn(
                                    "bg-white border rounded-xl overflow-hidden transition-all cursor-pointer hover:border-brand-200",
                                ...
                                <div className="p-4 flex items-center gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <Badge className={cn("text-[8px] font-bold border px-2 py-0.5 rounded-xl", typeStyle.bg, typeStyle.text, typeStyle.border)}>
                                                {draw.draw_type}
                                            </Badge>
                                            {isLatest && (
                                                <Badge className="text-[8px] font-bold bg-brand-600 text-white border-none px-2 py-0.5 rounded-xl">
                                                    LATEST
                                                </Badge>
                                            )}
                                            {draw.draw_round && (
                                    ...
                                    <div className="flex items-center gap-3 shrink-0">
                                        {draw.eligible_count > 0 && (
                                            <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 px-2.5 py-1.5 rounded-xl">
                                                ...
                                                <span className="text-[10px] font-bold text-emerald-700">{draw.eligible_count} Eligible</span>
                                            </div>
                                        )}
                                        {draw.near_eligible_count > 0 && (
                                            <div className="flex items-center gap-1.5 bg-amber-50 border border-amber-100 px-2.5 py-1.5 rounded-xl">
                                                ...
                                                <span className="text-[10px] font-bold text-amber-700">{draw.near_eligible_count} Near</span>
                                            </div>
                                        )}
                                ...
                                {isExpanded && (draw.eligible_clients.length > 0 || draw.near_eligible_clients.length > 0) && (
                                    <div className="border-t border-slate-100 bg-slate-50 p-4 space-y-3">
                                        {draw.eligible_clients.length > 0 && (
                                            <div>
                                                <p className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                                                    ...
                                                    {draw.eligible_clients.map(c => (
                                                        <div key={c.urn} className="flex items-center gap-1.5 bg-white border border-emerald-100 rounded-xl px-2.5 py-1.5">
                                                            <div className="w-4 h-4 rounded bg-emerald-100 flex items-center justify-center">
                                                                <span className="text-[8px] font-bold text-emerald-700">{c.full_name?.charAt(0)}</span>
                                                            </div>
                                                            <span className="text-[10px] font-bold text-slate-700">{c.full_name}</span>
                                                            <span className="text-[9px] font-bold text-emerald-600">CRS {c.crs_score}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        {draw.near_eligible_clients.length > 0 && (
                                            <div>
                                                <p className="text-[9px] font-bold text-amber-600 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                                                    ...
                                                    {draw.near_eligible_clients.map(c => (
                                                        <div key={c.urn} className="flex items-center gap-1.5 bg-white border border-amber-100 rounded-xl px-2.5 py-1.5">
                                                            <div className="w-4 h-4 rounded bg-amber-100 flex items-center justify-center">
                                                                <span className="text-[8px] font-bold text-amber-700">{c.full_name?.charAt(0)}</span>
                                                            </div>
                                                            <span className="text-[10px] font-bold text-slate-700">{c.full_name}</span>
                                                            <span className="text-[9px] font-bold text-amber-600">CRS {c.crs_score} ({draw.min_crs - c.crs_score} pts away)</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        {draw.eligible_clients.length === 0 && draw.near_eligible_clients.length === 0 && (
                                            <p className="text-[10px] text-slate-400 font-bold">No clients in your pool match this draw's criteria.</p>
                                        )}
                                    </div>
                                )}
                            </motion.div>
