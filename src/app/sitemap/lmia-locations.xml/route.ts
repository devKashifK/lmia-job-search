...
export async function GET() {
    ...
    try {
        // Use RPC to fetch all unique territories from lmia
        const { data, error } = await (db as any).rpc('get_lmia_table_distinct_values', {
            p_column_name: 'territory_norm'
        });
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url>
    <loc>${baseUrl}/lmia-jobs-in/${toSlug(loc)}?value=${encodeURIComponent(loc)}</loc>
    ...
    <priority>0.7</priority>
        ...
        console.error('Sitemap Error (lmia-locations):', error);
