...
export async function GET() {
    ...
    try {
        // Use RPC to fetch all unique NOC codes from lmia
        const { data, error } = await (db as any).rpc('get_lmia_table_distinct_values', {
            p_column_name: 'noc_code_norm'
        });
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ${uniqueNocs.map(noc => `
  <url>
    <loc>${baseUrl}/lmia-jobs?noc=${noc}</loc>
    ...
    <priority>0.7</priority>
        ...
        console.error('Sitemap Error (lmia-noc-codes):', error);
